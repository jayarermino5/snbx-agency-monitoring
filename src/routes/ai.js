  ...
  try {
    ...
    while (true) {
      ...
      if (data.status !== 'success') {
        return res.status(502).json({ error: 'GHL AI API error', raw: data });
      }
      all = all.concat(data.data);
      console.log(`[ai] fetched ${all.length}, hasMore=${data.hasMore}`);
      if (!data.hasMore || data.data.length < limit) break;
      skip += limit;
    }

    res.json({ success: true, companyId, startDate, endDate, locationsCount: all.length, data: all });
  } catch (err) {
    next(err);
  }
});
